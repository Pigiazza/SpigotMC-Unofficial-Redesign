...
export default function Wiki() { return <main className="sub-page"><SimpleHeader active="wiki"/><section className="wiki-hero"><BookOpen size={34}/><span className="section-kicker">DOCUMENTATION</span><h1>Everything you need<br/>to build with <em>Spigot.</em></h1><label><Search size={20}/><input placeholder="Search the documentation…"/><kbd>⌘ K</kbd></label></section><section className="wiki-shell"><aside><span>CONTENTS</span><a className="active" href="#start">Getting started</a><a href="#server">Server</a><a href="#plugins">Plugin development</a><a href="#api">Spigot API</a><a href="#proxy">BungeeCord</a><a href="#community">Community</a></aside><div><header><h2>Explore the Wiki</h2><p>Official guides, technical references, and community-curated answers.</p></header><div className="guide-grid">{guides.map(({ Icon, ...g }) => <article key={g.title}><div><Icon size={23}/><span>{g.tag}</span></div><h3>{g.title}</h3><p>{g.text}</p><a href="#read">Read the guide <ArrowRight size={14}/></a></article>)}</div></div></section></main> }
